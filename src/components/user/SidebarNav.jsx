...
const getNavItems = (orgId) => [
  ...
  },
  {
    ...
    url: `/${orgId}/dashboard/bill-upload`,
    icon: <UploadCloud className="w-5 h-5" />,
  },
  {
    title: 'Bill Intake & Prep',
    url: `/${orgId}/dashboard/bill-Intake-Prep`,
    icon: <ClipboardList className="w-5 h-5" />,
  },
  {
    title: 'Reports & Analytics',
    url: `/${orgId}/dashboard/reports`,
    icon: <BarChart3 className="w-5 h-5" />,
  },
  {
    title: 'Payment Management',
    url: `/${orgId}/dashboard/payments`,
    icon: <CreditCard className="w-5 h-5" />,
  },
];

export default function SidebarNav({ orgId, pathname }) {
  const items = getNavItems(orgId);
  return (
