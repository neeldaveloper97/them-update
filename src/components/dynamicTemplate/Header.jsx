...
export default function PublicHeader() {
  ...
  useEffect(() => {
    ...
    if (matchedOrg && matchedOrg.pages?.services) {
      const navData = matchedOrg.header.mainNav;
      setHeaderData(navData || null);
    } else {
      setNotFoundOrg(true);
    }
  }, [orgId]);
    ...
    <>
      ...
      <div className="sticky top-0 z-50 py-4 bg-white second_bar">
        <div className="container flex items-center justify-between px-4">
          <Link
            href={'/'}
            className="text-2xl font-semibold leading-relaxed text-primary-dark"
          >
            {/* {headerData?.logo} */}
            <img src="/images/them_logo.svg" alt="" className="w-14" />
          </Link>
          ...
          <nav className="items-center hidden xl:flex xl:gap-6 lg:gap-8">
            {[
              { href: `/${orgId}/about-us`, label: 'About us' },
              { href: `/${orgId}/what-we-do`, label: 'What We Do' },
              { href: `/${orgId}/faq`, label: 'FAQ' },
              { href: `/${orgId}/contact-us`, label: 'Contact Us' },
              {
                href: `/${orgId}/terms-and-conditions`,
            ...
            {hasMounted && !isLoggedIn && (
              <div className="flex items-center gap-2 lg:gap-3">
                <Button
                  asChild
                  variant="outline"
                  className="h-auto text-xs lg:text-sm"
                >
                  <Link href={`/${orgId}/register`} aria-label="Register">
                    Register
                  </Link>
                </Button>
                <Button asChild className="h-auto text-xs lg:text-sm">
                  <Link href={`/${orgId}/login`} aria-label="Login">
                    Login
                  </Link>
                </Button>
              </div>
          ...
          {hasMounted && !isLoggedIn && (
            <div className="flex flex-col pt-2 space-y-2 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-3">
              <Button
                asChild
                variant="outline"
                className="justify-center w-full bg-white sm:w-auto"
              >
                <Link href={`/${orgId}/register`} aria-label="Register">
                  Register
                </Link>
